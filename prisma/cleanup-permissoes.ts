 ...
 */
async function main() {
    const NOMES_OFICIAIS = ['CADASTRAR_USUARIOS','CADASTRAR_LIVROS','LOCAR_LIVROS','DEVOLVER_LIVROS','EXCLUIR_LIVROS','ACESSAR_DASHBOARD'];
    ...
    console.log('\n=== Estado final ===');
    final.forEach((u) =>
        console.log(`  id=${u.id}  ${u.email}  [${u.perfil}]  perms=[${u.permissoes.map((p) => p.nome).join(', ')}]`),
    );
}
